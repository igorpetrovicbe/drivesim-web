...
loaded_state_large = torch.load('weights/simulator_200M.pth')
loaded_state_medium = torch.load('weights/simulator_45M.pth')
loaded_state_small = torch.load('weights/simulator_15M.pth')
...
d_model_large = 768
ff_size_large = 4 * d_model_large
output_size = img_vocab_size + 2 * quantization_bins
num_layers_large = 12
num_heads_large = int(d_model_large / 64)

d_model_medium = 512
ff_size_medium = 4 * d_model_medium
num_layers_medium = 6
num_heads_medium = int(d_model_medium / 64)

d_model_small = 384
ff_size_small = 4 * d_model_small
num_layers_small = 4
num_heads_small = int(d_model_small / 64)

vqvae = VQVAE(**vqvae_args).to(device)

# Large simulator
simulator_large = MultiLayerTransformer(d_model_large, total_vocab_size, ff_size_large, num_layers_large, num_heads_large, input_sequence_length,
                              context_sequence_length)
simulator_large.load_state_dict(loaded_state_large['model_state_dict'])
simulator_large.to(device)
total_params = sum(p.numel() for p in simulator_large.parameters())
print(f"Total number of parameters in the large model: {total_params}")

# Medium simulator
simulator_medium = MultiLayerTransformer(d_model_medium, total_vocab_size, ff_size_medium, num_layers_medium, num_heads_medium, input_sequence_length,
                              context_sequence_length)
simulator_medium.load_state_dict(loaded_state_medium['model_state_dict'])
simulator_medium.to(device)
total_params = sum(p.numel() for p in simulator_medium.parameters())
print(f"Total number of parameters in the medimum model: {total_params}")

# Small simulator
simulator_small = MultiLayerTransformer(d_model_small, total_vocab_size, ff_size_small, num_layers_small, num_heads_small, input_sequence_length,
                              context_sequence_length)
simulator_small.load_state_dict(loaded_state_small['model_state_dict'])
simulator_small.to(device)
total_params = sum(p.numel() for p in simulator_small.parameters())
print(f"Total number of parameters in the small model: {total_params}")
...
vqvae.load_state_dict(torch.load('weights/vqvae_v4.pth'))

msgan = MSG_Generator().to(device)
loaded_generator_state = torch.load('weights/msgan_v4.pth')
msgan.load_state_dict(loaded_generator_state)

simulator_small.eval()
simulator_medium.eval()
simulator_large.eval()
#vqvae.eval()
#msgan.eval()
...
input_image_width = 544
input_image_height = 136
...
def generate_integer_sequence(img_list, speed, angle, num_frames, simulator, temperature=1.0):
    ...
    with torch.no_grad():
        for i in range(num_frames):
            ...
            encoder_out = simulator.encode(input_tensor)
            for j in range(img_length + 2):
                ...
                output = simulator.decode(encoder_out, context_tensor)
...
def generate_frames(image, angle, speed, num_frames, model_size, crop_image):
    if crop_image:
        cam_res_w = 1164
        cam_res_h = 874
        scale_factor = 874 / image.shape[0]
        new_w = int(image.shape[1] * scale_factor)
        new_h = int(image.shape[0] * scale_factor)
        image = cv2.resize(image, (new_w, new_h))
        # Center crop
        image = image[:, new_w // 2 - cam_res_w // 2:new_w // 2 + cam_res_w // 2]

        image = cv2.resize(image, (cam_res_w, cam_res_h))

        percent_crop = 0.33
        crop_amount = int(image.shape[0] * percent_crop)
        image = image[crop_amount:-crop_amount]

    image = cv2.resize(image, (input_image_width, input_image_height))
    cv2.imwrite(f'generated_images/-1.png', cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
    ...
    if model_size == "SMALL":
        ...
        simulator = simulator_medium
    else:
        simulator = simulator_large

    #TODO: Proveri koliko ima kanala 1,2,3,milion
    image = image[:, :, :3]
    ...
    with torch.no_grad():
        ...
        for k in range(len(image_indices) // img_length):
            ...
            if False and k > 0:
                angle = dequantize_angle(angle_indices[k-1])
                speed = dequantize_spd(spd_indices[k-1])
            ...
            generated_frames.append(image_np)
            cv2.imwrite(f'generated_images/{k}.png', image_np)
